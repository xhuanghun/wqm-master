...
<div id="monitorDataDetail_container" style="width:100%;height:100%;">
	...
	<div id="monitorDataDetailTabs" class="easyui-tabs" style="width:100%;height:100%;">
		...
		<table id="baseInfoTable" style="width:99%;cellspacing:0;cellpadding:0;border-collapse:collapse;" align="center"  border="0" cellspacing="0" >
			<tr>
				<td width="50%">水库名称:</td><td width="50%">${mediumReservoir.name} </td>
			</tr>
			<tr>
				<td width="50%">采样点位:</td><td width="50%">${mediumReservoir.monitorPoit} </td>
			</tr>
			<tr>
				<td width="50%">水库功能:</td><td width="50%">${mediumReservoir.content} </td>
			</tr>
			<tr>
				<td width="50%">所在区域:</td><td width="50%">${mediumReservoir.area.name} </td>
			</tr>
...
$('#monitorDataTable').datagrid({
    url:"${ctx}/mediumReservoir/getMonitorData?code="+$('#code').val(),
    method:'get',
    pagination:false,
    columns:[[
        {field:'id',title:'序号'},
    ...
    onLoadSuccess: function (rowData) {
    	mergeCellsByField('monitorDataTable', 'mediumReservoirName');
    },
});
/**
 * 设置分页
 ...
 */
 $("#monitorTime").combobox({
	    url:'${ctx}/mediumReservoir/getMonitorTime?code='+$('#code').val(),
	    valueField:'id',
	    textField:'time',
	    method:'GET',
